...
export default function bss({
  ...
  const cache = Map()
  const supportsVar = doc.documentElement.style && typeof CSS !== 'undefined' && CSS.supports('color', 'var(--var)')
  ...
  b.raw = bss(true)
  b.count = 0
  b.root = style.sheet.rules[style.sheet.insertRule(':root{}')]
  ...
  function bss(root) {
    return function b(xs, ...args) {
      const x = cache.get(xs) || instance(root)
      x.classes = [x.name]
      if (cache.has(xs)) {
  ...
  function ssr(str) {
    return {
      sheet: '<style class="bss_style" id="' + b.prefix + '">' + b.rules.join(b.prefix + '{}') + '</style>',
  ...
  function instance(root) {
    const x = {
      name: root ? '__raw' : (b.prefix + ++b.count),
      classes: [],
  ...
  function updateVar(x, prop, value) {
    ...
    if (x.value !== value) {
      x.value = value
      supportsVar
        ? b.root.style.setProperty('--' + x.name, value)
  ...
  function insert(rule) {
    ...
    if (b.dev && style) {
      ...
      try {
        style.sheet.insertRule(replaceVars(rule, vars, index), index)
      } catch (e) {
        b.debug && console.log('Ignored error:', e)
      }
    }
  ...
  function parseAddProps(xs, i, name, props, level) { // eslint-disable-line
    ...
    content && xs.push(
      (
       (!level.length || level[0][0] !== '&') && (!xs.selector || xs.selector.indexOf('@keyframes ')) !== 0 && name !== '__raw'
