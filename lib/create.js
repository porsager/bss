...
export default function bss({
  ...
  function b(xs, ...args) {
    ...
    for (let i = 1; i < xs.length; i++) {
      ...
      if (!classes(x, value)) {
        ...
        s += setVar(x.name + '-' + i, value, prop, suffix(prop, s)) + xs[i]
      }
    }

    x.rules = parse(removeComments(s).trim(), x).map(insert)
  ...
  function parseAddProps(xs, i, name, props, level) { // eslint-disable-line
    ...
    content && xs.push(
      ( (!level.length || level[0][0] !== '&') && (!xs.selector || xs.selector.indexOf('@keyframes ')) !== 0 ? '.' + name + (level.length ? ' ' : '') : '')
      + level.join(' ').replace(/&/g, '.' + name)
      + '{' + content + ';}'
    )
    props.end = i + 1
    props.out = []
