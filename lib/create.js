...
export default function bss({
  ...
  sheet = doc && typeof doc.querySelector === 'function' && doc.querySelector('.bss_style'),
  prefix = sheet && sheet.getAttribute('id') || randomId(),
  charset
} = {}) {
  const cache = Map()
  ...
  b.count = 0
  b.rules = sheet && sheet.textContent.split(b.prefix + '{}') || []
  b.rules.vars = []
  b.vars = {}
  b.debug = false
  b.dev = false
  ...
  function ssr(str) {
    ...
    const rules = b.rules.slice(0, index)
      .concat(
        (str.match(new RegExp(b.prefix + '[0-9]+', 'g')) || []).map(x =>
          b.rules[index + parseInt(x.slice(b.prefix.length))]
        )
      ).join(b.prefix + '{}')
